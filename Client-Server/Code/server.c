/* FinalProject server file submitted by Viutika Rathod 110094621 Section 3*/
...
int connectns_handld_by_server = 0;
int connectns_handld_by_mirror = 0;
...
// Function to check if a file has already been encountred
int isFilEncountred(const char* fil_nme) {
    for (int l = 0; l < num_encountred_files; l++) {
        if (strcmp(encountred_files[l], fil_nme) == 0) {
            return 1; // File is encountred
        }
    }
    return 0; // File is not encountred
}

// Function to mark a file as encountred
...
// Function to check whether tar file exists
int file_exists(const char *filename) {
    return access(filename, F_OK) == 0;
}

// Function to remove already existing zipped and unzipped file 
...
void redirect_to_mirror(int client_fd_markd) { 

    // Preparng redirectn mesage contaning mirror port
    char redirct_msg[1024];
    snprintf(redirct_msg, 1024, "%d\n", MIRROR_P_ORT);
    // Sendng redirection message to the client
    send(client_fd_markd, redirct_msg, strlen(redirct_msg), 0);
    close(client_fd_markd); //    // Closng client socket connection
}
...
int main(int arg_c, char const *arg_v[]) {
    ...
    int opt_info = 1;
    int addrlen_info = sizeof(addres_info);
    int connectns_handld_by_server = 0;
    int connectns_handld_by_mirror = 0;
    ...
    if (setsockopt(server_fd_info, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT, &opt_info, sizeof(opt_info))) {
        perror("setsockopt");
        exit(EXIT_FAILURE);
    }
    ...
    while (1) {
        ...
        if ((new_sockt_info = accept(server_fd_info, (struct sockaddr *)&addres_info, (socklen_t*)&addrlen_info)) < 0) {
            perror("accept");
            exit(EXIT_FAILURE);
        }
        ...
        if (total_connections <= 12) {
            if (total_connections <= 6) {
                // Handlng by server for first 6 connectns
                printf("New client connected to the server. Forking child process...\n");
                pid_t prcs_id = fork();
                if (prcs_id == -1) {
                    perror("fork");
                    exit(EXIT_FAILURE);
                    ...
                    exit(0); // Child process should exit after handlng client
                } else {
                    close(new_sockt_info);
                    connectns_handld_by_server++;
                    while (waitpid(-1, NULL, WNOHANG) > 0);
                }
            } else {
                // Handlng by mirror for next 6 connectns
                printf("New client connected to the mirror. Redirecting...\n");
                redirect_to_mirror(new_sockt_info);
                close(new_sockt_info);
                connectns_handld_by_mirror++;
            }
        } else {
            if (total_connections % 2 == 1) {
                ...
                if (prcs_id == -1) {
                    ...
                    close(server_fd_info);
                    prcs_client_func(new_sockt_info);
                    exit(0); // Child process should exit after handling the client
                } else {
                    close(new_sockt_info);
                    connectns_handld_by_server++;
                    while (waitpid(-1, NULL, WNOHANG) > 0);
